...
<body>
  <x-app-layout>
    ...
    <div class="card">
        <h3>{{ isset($informasidetail) ? 'Edit Informasi' : 'Tambah Informasi' }}</h3>
        <form action="{{ isset($informasidetail) ? route('admin.informasi.update', ['id' => $informasidetail->id]) : route('admin.informasi.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if(isset($informasidetail))
                @method('PUT')
            @endif
            <div class="form-group">
                <input type="text" name="judul" placeholder="Judul Informasi" value="{{ isset($informasidetail) ? $informasidetail->judul : old('judul') }}" required>
            </div>
            <div class="form-group">
                <textarea name="deskripsi" rows="10" placeholder="Deskripsi" required>{{ isset($informasidetail) ? $informasidetail->deskripsi : old('deskripsi') }}</textarea>
            </div>
            <div class="form-group">
                <input type="file" name="gambar" accept="image/*" @if(!isset($informasidetail)) required @endif>
            </div>
    ...
    <div class="info-card">
      <img src="{{ asset('uploads/' . $info->gambar) }}" alt="{{ $info->judul }}"
        ...
        <div class="button-group">
          ...
          <form action="{{route('admin.informasi.delete', ['id' => $info->id])}}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus informasi ini?');">
            @csrf
            @method('DELETE')
